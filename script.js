...
const productData = [
	...
	{
		id: 8,
		name: "Clean and soft sterile disposable men's underwear",
		price: 110.55,
...
const inventory = [
	...
	{ id: 8, name: "Men's Underwear", price: 110.55, stock: 11 },
...
// =====================
// PRODUCT CARD HIGHLIGHT ON CLICK
// =====================
...
promoButtons[0]?.addEventListener("click", () =>
	filterCards((card) => getPrice(card) <= 20)
);
promoButtons[1]?.addEventListener("click", () =>
	filterCards((card) => getPrice(card) >= 20 && getPrice(card) <= 60)
);
...
const products = [
	...
	{ id: 8, name: "Men's Underwear", price: 110.55 },
...
const findProduct = (id) => products.find(({ id: pid }) => pid === id);
...
const checkAllStock = () => {
	...
	inventory.forEach((product) => console.log(getStockStatus(product)));
	console.log("--------------------------------");
...
console.log(
	" showProducts() | 🛒 calculatePrice(id, qty) |  applyDiscount(amt)"
);
...
// =====================
// FETCH LIVE EXCHANGE RATE HELPER
// =====================
...
const fetchProducts = async () => {
	...
	try {
		...
		if (!productResponse.ok)
			throw new Error(`Server error: ${productResponse.status}`);
		...
		container.innerHTML = fetchedProducts
			.map(
		...
		<div class="fetched-card" data-id="${id}">
		  ...
		  <button class="cart-btn" onclick="addFetchedToCart(this)">Add to Cart</button>
		</div>
	  `
			)
			.join("");
...
const loadProductDetail = async (productId) => {
	...
	try {
		...
		modalContent.innerHTML = `<p> Failed to load product details. Try again.</p>`;
...
// =====================
const loadLocalProductDetail = (productId) => {
...
document.addEventListener("click", (e) => {
	// Fetched card click
	const fetchedCard = e.target.closest(".fetched-card");
	if (fetchedCard && !e.target.classList.contains("cart-btn")) {
		loadProductDetail(fetchedCard.dataset.id);
		return;
	}

	// Local card click
	const localCard = e.target.closest(".product-card");
	if (localCard && !e.target.classList.contains("cart-btn")) {
		loadLocalProductDetail(parseInt(localCard.dataset.id));
	}
});
